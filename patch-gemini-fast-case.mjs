...
const replacement=`  // O Gemini gera somente o núcleo narrativo. Mapa, dispositivos e câmeras são derivados
  // deterministicamente no servidor para reduzir latência, quota e risco de JSON truncado.
  const coreSchema:any={...caseSchema,required:(caseSchema.required||[]).filter((x:string)=>x!=='world'),properties:{...(caseSchema as any).properties}};
  ...
  const locDefs=[
    ['local-principal','Local principal','scene','Área central ligada ao incidente.'],
    ['acesso','Área de acesso','access','Entradas, saídas e circulação do local.'],
    ['administrativo','Área administrativa','office','Registros, documentos e infraestrutura institucional.'],
    ['externo','Ponto externo','outside','Área externa relevante para deslocamentos e observação.'],
    ['secundario','Local secundário','secondary','Local associado às relações dos envolvidos.']
  ];
