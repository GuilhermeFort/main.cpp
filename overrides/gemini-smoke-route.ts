...
export async function GET(req:Request){
  try{
    ...
    const code=(url.searchParams.get('code')||'HELH3C').trim().toUpperCase();
    ...
    return Response.json({ok:true,title:mystery.title,characters:mystery.characters.length,clues:mystery.clues.length,hasWorld:!!(mystery as any).world});
  }catch(error){
